...
const defaults = {
  week: 1,
  season: 2026,
  awards: { tryHard: "Taking Back NFL Sunday", lowestScore: "Jets Running Hot Oil", belted: "Kasnas Cheifs" },
  output: path.join(__dirname, "..", "artifacts", "mock-scoreboard.png"),
  matchups: [
...
const rows = matchups.map((m, i) => {
  ...
  const rightWon = Number(m[4]) > Number(m[1]);
  return `
  <div class="matchup">
    ...
    <strong class="score ${rightWon ? "winner" : ""}">${m[4]}</strong>
...
</style></head><body><main class="card"><header><div><div class="eyebrow">NJL FANTASY FOOTBALL</div><div class="title">WEEKLY SCOREBOARD</div></div><div class="week">WEEK ${input.week} • FINAL</div></header>${rows}<section class="awards"><div class="award try"><b>TRY HARD</b><span>${input.awards.tryHard}</span></div><div class="award nerd"><b>LOWEST SCORE</b><span>${input.awards.lowestScore || input.awards.jackass}</span></div><div class="award belt"><b>BELTED</b><span>${input.awards.belted}</span></div></section><footer>NJL SCORECENTER • ${input.season} SEASON</footer></main></body></html>`;
...
(async () => {
  ...
  await page.locator(".card").screenshot({ path: input.output || defaults.output });
  await browser.close();
})();
